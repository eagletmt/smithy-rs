...
data class RustDependency(
    ...
    override fun toString(): String {
        ...
        attribs.add(
            when (location) {
                is CratesIo -> """version = "${location.version}""""
                is Local -> """path = "${location.path}/$name""""
            }
        )
        with(features) {
            if (!isEmpty()) {
    ...
    companion object {
        ...
        // TODO: this should be vendored / autogenerated; 1d
        fun SerdeUtils(runtimeConfig: RuntimeConfig) = RustDependency(
            "serde-utils", Local(runtimeConfig.relativePath)
        )
